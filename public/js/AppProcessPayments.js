Vue.component('app-process-payments', {
  template:`
    <div>
      <div class="x_panel">
        <div class="x_title">
                ...
                <div class="table">
                  <table class="table table-striped jambo_table bulk_action small text-right">
                    ...
                    <tr v-for="invoice of $store.state.client_invoices">
                      ...
                      <td>{{ new Intl.NumberFormat("es-ES", {maximumFractionDigits: 2}).format(invoice.iva_bolivar) }}</td>
                      <td>{{ new Intl.NumberFormat("es-ES", {maximumFractionDigits: 2}).format(invoice.iva_dollar) }}</td>
                      <td>{{ new Intl.NumberFormat("es-ES", {maximumFractionDigits: 2}).format(invoice.factor) }}</td>
...
const store = new Vuex.Store({
  ...
  mutations:{
    ...
    getInvoicesPayment:function(state) {
      axios
        .get(this._vm.$api_url + 'clients/' + state.client_select.id + '/payment/iva')
        .then(response => {
...
/*
      for(invoices_to_pay of state.invoices_to_pay){
        index = 0
        for(invoice of invoices){
          if(invoices_to_pay.id == invoice.id){
            state.amount_charged += invoice.to_pay_bolivar
            break
          }
        }
      }
